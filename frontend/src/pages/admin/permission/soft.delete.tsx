...
import { callDeletePermission, callRestorePermission } from "@/config/api";
...
const PermissionDeleted = () => {
    ...
    const isFetching = useAppSelector(state => state.permissionDeleted.isFetching);
    const meta = useAppSelector(state => state.permissionDeleted.meta);
    const permissions = useAppSelector(state => state.permissionDeleted.result);
    const dispatch = useAppDispatch();

    const handleRestorePermission = async (id: string | undefined) => {
        if (id) {
            const res = await callRestorePermission(id);
            if (res && res.statusCode === 200) {
                message.success('Restore permission successfully');
                reloadTable();
                dispatch(fetchPermission({ query: "?page=1&size=10&sort=updatedAt,desc" })); // Refresh deleted users tab
            } else {
                notification.error({
                    message: 'Error occur',
                ...
                <Space>
                    ...
                    >
                        <ClockCircleOutlined
                            ...
                            type=""
                            onClick={() => { handleRestorePermission(entity.id) }} />
                    </Access >
                </Space>
            ),
            ...
            >
                <DataTable<IPermission>
                    ...
                    request={async (params, sort, filter): Promise<any> => {
                        ...
                        dispatch(fetchPermissionDeleted({ query }))
                    }}
                    scroll={{ x: true }}
                    pagination={
                        {
...
export default PermissionDeleted;